...
/**
 * Servlet implementation class SkillBuilderEdmodoClassroom
 */
@WebServlet("/SkillBuilderEdmodoClassroom")
public class SkillBuilderEdmodoClassroom extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SkillBuilderEdmodoClassroom() {
        super();
...
//		if(externalRef == null){
//			request.getRequestDispatcher("edmodo_landing_page.jsp").forward(request, response);
//			return;
//		}
...
//		List<Integer> folderIdList = Arrays.asList(folderIds);
		if(folders.size() != 0){
			...
			request.getRequestDispatcher("skill_builder_edmodo_classroom.jsp").forward(request, response);
		}else{
//			String folderName = pss.getFolderNameById(Long.parseLong(folderId));
			...
			request.getRequestDispatcher("ap_edmodo_classroom.jsp").forward(request, response);
		}
	}
