 ...
 */
@WebServlet("/SkillBuilderSchoologyClassroom")
public class SkillBuilderSchoologyClassroom extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SkillBuilderSchoologyClassroom() {
        super();
...
//		List<Integer> folderIdList = Arrays.asList(folderIds);
		if(folders.size() != 0){
			...
			request.getRequestDispatcher("skill_builder_schoology_classroom.jsp").forward(request, response);
		}else{
//			String folderName = pss.getFolderNameById(Long.parseLong(folderId));
			...
			request.getRequestDispatcher("ap_schoology_classroom.jsp").forward(request, response);
		}
